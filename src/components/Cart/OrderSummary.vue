...
export default {
    ...
    computed:{
        total() {
            ...
            return Number(totalValue).toLocaleString('en-In',{style:'currency',currency:"INR"});
        },
        discount() {
            let totalDiscount = this.$props.product.reduce((current,previous)=>{
                return current + ((previous.originalPrice * previous.quantity) - (previous.price * previous.quantity))
            },0)
            return Number(totalDiscount).toLocaleString('en-In',{style:'currency',currency:"INR"});
        },
        cartValue() {
            let cart = this.$props.product.reduce((current,previous)=>{
                return current + (previous.price * previous.quantity)
            },0)
            return Number(cart).toLocaleString('en-In',{style:'currency',currency:"INR"});
        }
    }
}
</script>
